KISSY.add('canvax/animation/AnimationFrame', function () {
    /**
...
;KISSY.add('canvax/display/Text', function (S, DisplayObject, Base) {
     ...
     * 删除事件侦听
     */
    CanvaxEvent.removeEvent = addOrRmoveEventHand('removeEventListener', 'detachEvent');    //阻止浏览器的默认行为 
    //阻止浏览器的默认行为 
    ...
    return EventBase;
}, { requires: ['canvax/core/Base'] });;KISSY.add('canvax/event/EventDispatcher', function (S, Base, EventManager) {
        ...
        _removeEventListener: function (type, listener) {
            ...
            for (var i = 0; i < map.length; i++) {
                ...
                if (li === listener) {
                    map.splice(i, 1);
                    if (map.length == 0) {
                        delete this._eventMap[type];    //如果这个如果这个时候child没有任何事件侦听
    ...
    return EventManager;
}, { requires: [] });;KISSY.add('canvax/geom/HitTestPoint', function (S, Base, myMath) {
    /**
     * 图形空间辅助类
     * isInside：是否在区域内部
     * isOutside：是否在区域外部
    ...
    function _buildPathMethod(shape, context, x, y) {
        var context = shape.context;    // 图形类实现路径创建了则用类的path
        // 图形类实现路径创建了则用类的path
        context.beginPath();
    ...
    function _isInsideRing(shape, x, y) {
        var context = shape.context;
        if (_isInsideCircle(shape, x, y) && !_isInsideCircle(shape, x, y, context.r0 || 0)) {
            // 大圆内，小圆外
            return true;
        }
        return false;
    ...
    function _isInsideSector(shape, x, y) {
        ...
        if (!_isInsideCircle(shape, x, y) || context.r0 > 0 && _isInsideCircle(shape, x, y, context.r0)) {
            ...
            //如果在start和end的数值中，end大于start而且是顺时针则regIn为true
...
&& // 在水平的边界线上
...
});;KISSY.add('canvax/geom/Math', function (S) {
    var _cache = {
            sin: {},
            //sin缓存
            cos: {}    //cos缓存
...
}, { requires: [] });;KISSY.add('canvax/geom/Matrix', function (S, Base) {
    ...
    return Matrix;
}, { requires: ['canvax/core/Base'] });;KISSY.add('canvax/geom/smoothSpline', function (S, Vector) {
    /**
     * @inner
     */
...
;
        ...
        //当前激活的点对应的obj，在touch下可以是个数组,和上面的curPoints对应
        this.curPointsTarget = [];    //每帧 由 心跳 上报的 需要重绘的stages 列表
        ...
        this._preRenderTime = 0;    //任务列表, 如果_taskList 不为空，那么主引擎就一直跑
        ...
        //比如Movieclip的__enterFrame方法。
        this._taskList = [];
        this._hoverStage = null;    //为整个项目提供像素检测的容器
        //为整个项目提供像素检测的容器
        this._pixelCtx = null;
        this._isReady = false;    /**
        *交互相关属性
    ...
    Base.creatClass(Canvax, DisplayObjectContainer, {
        init: function () {
            ...
            this.context.height = this.el.height();    //然后创建一个用于绘制激活shape的 stage到activation
            //然后创建一个用于绘制激活shape的 stage到activation
            this._creatHoverStage();    //初始化事件委托到root元素上面
            //初始化事件委托到root元素上面
       ...
       */
        ...
        * */
        __touchHandler: function (e) {
            var self = this;    //用hamer的方式来阻止执行浏览器默认事件
                ...
                if (e.type == 'dragstart') {
                    ...
                    _.each(self.curPointsTarget, function (child, i) {
                        if (child && child.dragEnabled) {
                            //只要有一个元素就认为正在准备drag了
                            self._draging = true;    //然后克隆一个副本到activeStage
                            //然后克隆一个副本到activeStage
                            self._clone2hoverStage(child, i);    //先把本尊给隐藏了
                            //先把本尊给隐藏了
        ...
        * */
        ...
        __getCanvaxPointInTouchs: function (e) {
            ...
            _.each(e.gesture.touches, function (touch) {
                touch.x = touch.pageX - self.rootOffset.left, touch.y = touch.pageY - self.rootOffset.top;
        ...
        * */
        __mouseHandler: function (e) {
            ...
            if (e.type == 'mousemove') {
                ...
                if (self._touching && e.type == 'mousemove' && curMouseTarget) {
                    ...
                    if (!self._draging) {
                        //begin drag
                        curMouseTarget.dragBegin && curMouseTarget.dragBegin(e);    //先把本尊给隐藏了
                        //先把本尊给隐藏了
                        curMouseTarget.context.visible = false;    //然后克隆一个副本到activeStage
                        //然后克隆一个副本到activeStage
        ...
        _dragHander: function (e, target, i) {
            ...
            target.drag && target.drag(e);    //要对应的修改本尊的位置，但是要告诉引擎不要watch这个时候的变化
            ...
            target._notWatch = false;    //同步完毕本尊的位置
        },
        //同步完毕本尊的位置
        //drag结束的处理函数
        _dragEnd: function (e, target, i) {
            var self = this;
            self.dragEnd && self.dragEnd(e);    //拖动停止， 那么要先把本尊给显示出来先
                                                //这里还可以做优化，因为拖动停止了但是还是在hover状态，没必要把本尊显示的。
                                                //_dragDuplicate 复制在_hoverStage 中的副本
            //拖动停止， 那么要先把本尊给显示出来先
            //这里还可以做优化，因为拖动停止了但是还是在hover状态，没必要把本尊显示的。
            //_dragDuplicate 复制在_hoverStage 中的副本
            var _dragDuplicate = self._hoverStage.getChildById(target.id);    //这个时候的target还是隐藏状态呢
            //这个时候的target还是隐藏状态呢
            target.context.visible = false;    //target._updateTransform();
        ...
        __enterFrame: function () {
            ...
            if (self._heartBeat) {
                ...
                //渲染完了，打上最新时间挫
                self._preRenderTime = new Date().getTime();    //渲染结束
                //渲染结束
        ...
        heartBeat: function (opt) {
            //displayList中某个属性改变了
            var self = this;    //心跳包有两种，一种是某元素的可视属性改变了。一种是children有变动
