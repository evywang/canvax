...
KISSY.add('canvax/shape/Beziercurve', function (S, Shape, Base, Point) {
    var Beziercurve = function (opt) {
        ...
        self.length = undefined;    //在diaplayobject组织context的之前就要把 这个 shape 需要的 style 字段 提供出来
     ...
     */
        getPointByTime: function (t) {
            ...
            return new Point(it3 * context.xStart + 3 * it2 * t * context.cpX1 + 3 * it * t2 * context.cpX2 + t3 * context.xEnd, it3 * context.yStart + 3 * it2 * t * context.cpY1 + 3 * it * t2 * context.cpY2 + t3 * context.yEnd);
        },
        /**
     * @param  {number} -- len
     ...
     */
        getPointByLen: function (len) {
            ...
            if (len > this._length) {
                return {
                    x: this.context.$model.xEnd,
                    y: this.context.$model.yEnd
                };
            }
            var t1 = len / this._length, t2;
            do {
